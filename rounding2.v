...
induction p.
  ...
  rewrite Rplus_assoc; apply Rplus_lt_compat.
    ...
    apply Rmult_le_0_lt_compat;[lt0 | lt0 | |].
      ...
      apply Rle_lt_trans with (6/7 * ((3 / 2) ^ n * local_e)).
  ...
  replace (A / (B + hsm) - A / B) with (- A * hsm / (B * (B + hsm))); cycle 1.
    ...
    apply Rlt_trans with (16 * (3 ^ n / 2 * local_e)).
    rewrite -> (Rmult_comm A), (Rmult_assoc hsm), (Rmult_comm hsm).
    rewrite Rabs_mult; apply Rmult_le_0_lt_compat; try apply Rabs_pos; auto.
    apply Rlt_le_trans with (Rabs A * 4); cycle 1.
    ...
    rewrite Rabs_mult; apply Rmult_lt_compat_l.              
      rewrite Rabs_mult; apply Rmult_lt_0_compat.
        now apply Rabs_pos_lt; psatzl R.
      apply Rabs_pos_lt, Rgt_not_eq, pow2_gt_0.
      now assert (tmp := u_n_vs2_lb _ n1); psatzl R.
    ...
    apply Rmult_le_0_lt_compat; try lt0.
      replace 2 with (/ / 2) by field.          
      now apply Rinv_lt_contravar; lt0.
    replace 2 with (/ / 2) by field.          
    now apply Rinv_lt_contravar; lt0.
  now psatzl R.
intros a b am1 bm1 sum twopk ha hb ha1 hb1 hsm n1 qa qb qu qv qs qtp
  intha inthb intha1 inthb1 abshsm inte clee.
simpl rsalamin_rec.
